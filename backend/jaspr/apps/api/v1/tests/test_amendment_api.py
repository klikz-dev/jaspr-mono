...
from jaspr.apps.kiosk.models import Amendment, Encounter, Patient, Technician
...
class TestTechnicianPatientAmendmentAPIPermissions(JasprTestResourcePermissions):
    ...
    def amendment_factory_for_this_test(self) -> Amendment:
    ...
    def setUp(self):
        super().setUp(
            resource_pattern="technician/encounter/{encounter_id}/amendments",
            version_prefix="v1",
            factory_name="amendment_factory_for_this_test",
        )

        self.encounter = self.create_patient_encounter(
            patient=self.patient, department=self.department
        )
        self.base_uri = self.base_uri.format(encounter_id=self.encounter.id)
        self.detail_uri = self.detail_uri.format(encounter_id=self.encounter.pk)
...
class TestTechnicianPatientAmendmentAPI(JasprApiTestCase):
    def setUp(self):
        super().setUp()
        self.uri = "/v1/technician/encounter/{encounter_id}/amendments/{amendment_id}"
        ...
        self.note_type = "narrative-note"

    def prepare_full_uri(self, encounter_id: int, amendment_id: int = None) -> str:
        prepared_url = self.uri.format(encounter_id=encounter_id, amendment_id=amendment_id)
    ...
    def make_amendment(self, **kwargs) -> Amendment:
        kwargs.setdefault("encounter", self.encounter)
        kwargs.setdefault("technician", self.technician)
        kwargs.setdefault("note_type", self.note_type)
    ...
    def test_technician_can_list_amendments_belonging_to_patients_current_encounter(
        self,
    ):
        ...
        self.make_amendment(
            encounter=other_patient_encounter, technician=self.technician, note_type=self.note_type
        )
        self.create_amendment()
        uri = self.prepare_full_uri(self.encounter.id)
        ...
        self.assertEqual(
            ...
            [
                {
                    "id": amendment2.id,
                    "note_type": amendment2.note_type,
                ...
                {
                    "id": amendment1.id,
                    "note_type": amendment1.note_type,
                ...
                {
                    "id": amendment3.id,
                    "note_type": amendment3.note_type,
    ...
    def test_technician_can_create_amendment(self):
        now = timezone.now()
        other_amendment = self.make_amendment(comment="Here's a comment.", note_type=self.note_type)
        uri = self.prepare_full_uri(self.encounter.id)
        response = self.client.post(uri, data={"comment": "Here's another comment.", "noteType": self.note_type})
    ...
    def test_technician_can_edit_amendment_that_technician_created(self):
        ...
        uri = self.prepare_full_uri(self.encounter.id, amendment.id)
        before_update = timezone.now()
        print("URI", uri)
        response = self.client.put(uri, data={"comment": "Here's an updated comment.", "noteType": self.note_type})
    ...
    def test_technician_can_delete_amendment_that_technician_created(self):
        now = timezone.now()
        with freeze_time(now - timedelta(seconds=1)):
            amendment = self.make_amendment(comment="Here's a comment.", note_type="narrative-note")
        uri = self.prepare_full_uri(self.encounter.id, amendment.id)
        before_update = timezone.now()
    ...
    def test_technician_cannot_edit_amendment_that_other_technician_created(self):
        ...
        amendment = self.make_amendment(
            technician=other_technician, comment="Here's a comment.", note_type=self.note_type
        )
        uri = self.prepare_full_uri(self.encounter.id, amendment.id)
        ...
        for case in ("list", "update"):
            with self.subTest(case=case):
                if case == "list":
                    ...
                    amendment = self.make_amendment(technician=technician, note_type=self.note_type)
                    uri = self.prepare_full_uri(self.encounter.id, amendment.id)
